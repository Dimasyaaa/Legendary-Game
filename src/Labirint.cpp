...
// ====================
const string WALL_SYMBOL = "█";     ///< Символ для стен
const string PLAYER_SYMBOL = "Ω";   ///< Символ игрока
const string TREASURE_SYMBOL = "$"; ///< Символ сокровища
const string PATH_SYMBOL = "○";     ///< Символ для путей
const string FOG_SYMBOL = "?";      ///< Символ тумана войны

// ==================== РЕАЛИЗАЦИЯ МЕТОДОВ КЛАССА GAME ====================

Game::Game()
    : player_posX(1), player_posY(1), treasure_X(0), treasure_Y(0),
      treasureSymbol(TREASURE_SYMBOL), playerSymbol(PLAYER_SYMBOL),
      isRunning(true) {
...
void Game::displayMap() {
  ...
  // TODO из файлика читать цвет и подставлять вместо 15/14 и т.д.
  for (int i = 0; i < n; i++) {
    for (int j = 0; j < m; j++) {
      // Видимая область
