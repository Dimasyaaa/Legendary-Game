...
public:
  Converter()
      : factor_power(1.0), factor_dexterity(1.0), factor_bodytype(1.0),
        factor_intelligence(1.0), factor_luck(1.0) {}
  // setters
  void set_factor_power(float value) { factor_power = value; }
  void set_factor_dexterity(float value) { factor_dexterity = value; }
  void set_factor_bodytype(float value) { factor_bodytype = value; }
  void set_factor_intelligence(float value) { factor_intelligence = value; }
  void set_factor_luck(float value) { factor_luck = value; }
  // geters
  float get_factor_power() { return factor_power; }
  float get_factor_dexterity() { return factor_dexterity; }
  float get_factor_bodytype() { return factor_bodytype; }
  float get_factor_intelligence() { return factor_intelligence; }
  float get_factor_luck() { return factor_luck; }
};
...
public:
  ...
  Item(const string &name, const string &description, float power,
       float dexterity, float speed)
      : name(name), description(description), power_item(power),
        dexterity_item(dexterity), speed_item(speed) {}
...
public:
  ...
  Character(const string &name, const string &type, float power,
            float dexterity, float bodytype, float intelligence, float luck)
      : name(name), type(type), power_Character(power),
        dexterity_Character(dexterity), bodytype_Character(bodytype),
        intelligence_Character(intelligence), luck_Character(luck) {}
...
public:
  ...
  Hero(const string &name, float power, float dexterity, float bodytype,
       float intelligence, float luck)
      : Character(name, "The main character", power, dexterity, bodytype,
                  intelligence, luck) {}
...
public:
  // Конструктор врага
  Enemy(const string &name, float power, float dexterity, float bodytype,
        float intelligence, float luck)
      : Character(name, "Enemy", power, dexterity, bodytype, intelligence,
                  luck) {}
