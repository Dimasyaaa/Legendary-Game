...
public:
  Camera(int coord_x, int coord_y, int zom)
      : camera_pos_x(coord_x), camera_pos_y(coord_y), zoom(zom) {}

  // Трансляция. Внутрь передаётся обьект трансляции - двумерный массив
  // символов. Количество выводимы символов соответствует формуле(max_map/zoom)
  // ( при максимальном размере
  //  карты 10, если зум равен 2 то выведится 5 символов. Если zoom  равен 3 то
  //  3)
  ...
  // метод для установки позиции камеры. Символы выводятся начиная с сивмовла
  // находящегося на позиции камеры
  void link(int coord_x, int coord_y) { // привязка камеры к точке
    if ((coord_x == clamp(coord_x, zoom / 2, max_map - 1 - zoom / 2)) &&
        (coord_y == clamp(coord_y, zoom / 2, max_map - 1 - zoom / 2))) {
      camera_pos_x = coord_x;
      camera_pos_y = coord_y;
    } else if ((coord_x != clamp(coord_x, zoom / 2, max_map - 1 - zoom / 2)) &&
               (coord_y == clamp(coord_y, zoom / 2, max_map - 1 - zoom / 2))) {
      camera_pos_y = coord_y;
    } else if ((coord_x == clamp(coord_x, zoom / 2, max_map - 1 - zoom / 2)) &&
               (coord_y != clamp(coord_y, zoom / 2, max_map - 1 - zoom / 2))) {
